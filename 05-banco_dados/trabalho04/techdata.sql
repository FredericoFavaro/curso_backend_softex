...
6. Agrupamento com a clausula GROUP BY: agrupe as transacoes por produto e calcule a media de valor para cada produto;
7. Consulta combinada: escreva uma consulta que selecione a quantidade total de produtos vendidos (COUNT), o valor total das vendas (SUM) e a media de valor por transacao, agrupados por categoria de produtos.
*/

-- Acredito que para a atividade esta faltando o banco de dados de exemplo
...
SELECT
    Produto,
    AVG(Valor) AS MediaValorProduto
FROM transacoes
GROUP BY Produto;

SELECT
    Categoria,
    COUNT(*) AS TotalProdutosVendidos,
    SUM(Valor) AS ValorTotalVendas,
    AVG(Valor) AS MediaValorTransacao
FROM transacoes
GROUP BY Categoria;